...
var Game= meta.objectify(function(initial_state) {
    var self = this;
    if (!initial_state)
	initial_state= {};
...
Game.method('add_player', function(player_id) {
    var new_player= new Player();
    this.players[player_id]= new_player;
    return new_player;
});

Game.method('connect_socket', function(player_id, socket) {
    this.players[player_id].socket = socket;
});

Game.method('connect_ship', function(player_id, ship) {
    var the_player= this.players[player_id];
    the_player.ship= ship;
    ship.player(the_player);
});

Game.method('add_ship', function(parameters) {
